...
OFFICE_REPORT_SCHEMA = {
    "name": "office_report",
    "description": (
        "Написать в общий рабочий чат команды от своего имени. "
        "Используй, когда закончил задачу, полученную от другого агента через "
        "доску Kanban: скажи, от кого была задача, в чём она состояла, и дай "
        "результат. Пиши обычным человеческим текстом, как коллега в чате."
    ),
    "input_schema": {
        "type": "object",
        "properties": {
...
def office_report(text: str = "", **kwargs) -> str:
    """Отправить сообщение в офисный чат ботом ЭТОГО профиля."""
    text = (text or "").strip()
    if not text:
