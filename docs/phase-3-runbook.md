...
### Защита ветки main — по факту не технический барьер (решили не платить)
Планировался настоящий технический барьер (`Settings → Branches → Add rule` → main → Require a pull request before merging), но выяснилось: **на приватных репозиториях GitHub это не работает на бесплатном тарифе** — ни классические branch protection rules, ни новые rulesets, оба требуют GitHub Pro (~$4/мес на личном аккаунте). Михаил решил не платить — сознательный выбор, не забытый шаг.

Что это значит на практике: запись в vault через PR держится **только** на инструкции в `SOUL.md`/`vault-pr-write` (агент сам решает не пушить в main) — не на технической блокировке. Прямой push от brain в main технически пройдёт, если агент вдруг соберётся с пути. Смягчающий фактор: git ничего не удаляет — даже случайный прямой push виден в `git log` и откатывается `git revert`, то есть худший случай — не потеря данных, а review постфактум вместо превью в PR. Если позже решишь всё-таки заплатить за Pro — просто вернись к этому шагу, ничего больше менять не нужно.
...
- [ ] Запись в vault идёт строго через PR по факту (агент следует SOUL.md/скиллу) — технической блокировки на бесплатном GitHub нет, решили не платить за Pro, см. раздел 1.
