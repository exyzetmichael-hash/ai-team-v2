...
⚠️ **Таймзона и «обёртка» cron-доставки теперь тоже в `config.yaml`** (шаг 1 уже копирует его). Без явной таймзоны Hermes падает на системную зону сервера — не на время Михаила — и `"0 8 * * *"` стреляет не в то время; в конфиге уже прописано `timezone: Asia/Krasnoyarsk` (Михаил в Красноярске — зону надо спрашивать у человека, а не выводить из планов на переезд), и `cron.wrap_response: false`, чтобы Telegram-сообщение приходило чистым текстом, а не в обёртке `Cronjob Response: ... To stop or manage this job...`. Если ты уже создавал задачу раньше (до этого фикса) — **удали её и создай заново**, старая могла запомнить неправильное время:
```bash
hermes -p secretary cron list                # найди job_id
hermes -p secretary cron remove <job_id>
```

Заведи cron-задачу. **Расписание — cron-выражением** (`минута час * * *`), а не словами: Hermes понимает `"30m"`, `"every 2h"` и cron-выражения, но **не** формы вида «every 1d at 08:00». Для «каждый день в 08:00» это `"0 8 * * *"` (теперь однозначно — 8 утра по времени Михаила):
```bash
