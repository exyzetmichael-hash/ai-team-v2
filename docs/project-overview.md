...
`on_session_end`, если к концу турна `office_report` так и не вызван, шлёт отчёт сам, с
пометкой «🔧 Автоотчёт». Оба хука ключуют состояние по `task_id or session_id` — важно, что
...
адресацию по всем семи ролям, негативные кейсы, слэш-команды, **гонку семи процессов за
арбитраж** (ровно один вызов модели, ровно один отвечающий, корректный фолбэк при сбое) и
страховку `office_report` (срабатывает при забытом вызове, не дублирует при вызванном, не шлёт
при ошибке `kanban_complete`).
