...
> ⚠️ **АРХИВ.** Документ описывает первую версию проекта — «офис» из семи ботов-личностей с
> координацией через Kanban (фазы 0-6). Технически всё описанное ниже работало и было
> проверено вживую, но сам формат не подошёл владельцу на практике — заменён на единого
> ассистента EDITH. Текущая архитектура: `profiles/README.md` (профиль, доступы) и
> `docs/phase-9-edith-runbook.md` (развёртывание). Оставлено как запись решений и найденных
> подводных камней Hermes — многое из технических деталей (провайдер, кэш, MCP, Kanban-баги)
> актуально независимо от архитектуры сверху.

Документ для технических специалистов: что построено, на чём, какие решения приняты и почему,
где границы и известные проблемы. Актуально на 2026-07-25 (до разворота на EDITH).
...
который пишет **ботом самого исполнителя** и **в его собственный топик** группы (константа
окружения профиля `OFFICE_GROUP_THREAD_ID`, а не аргумент — выбрать чужой топик нельзя ни
модели, ни коду). Пробовали адресовать в топик того, кто прислал задачу (`created_by`
карточки, через прямой SQLite-запрос к `kanban.db`) — работало корректно, но на практике
оказалось менее удобно, чем свой топик: с ним у каждого агента предсказуемое место для
результатов, без необходимости держать в голове цепочку «кто у кого что заказал». Откатили
обратно на свой топик. Сырые авто-уведомления отключены (`kanban.auto_subscribe_on_create:
false`).
...
дублирует при успешном) и саму сигнатуру `office_report(args, **kw)` под реальную calling
convention Hermes.
...
**~~Маркетплейс пойдёт через РФ-прокси, а не переездом стека.~~ ОПРОВЕРГНУТО 2026-07-29.**
Предполагалось, что Seller API WB/Ozon может блокировать EU-адреса, и закладывался маленький
российский VPS как прокси под маркетплейс-трафик. Проверка с боевого сервера показала, что
**блокировки нет**: и Ozon, и все шесть доменов WB отвечают `401` (то есть «дошло, нужен
ключ») за 0.05–0.12 с. Прокси не нужен, российский VPS под эту задачу не требуется.
Замеры и текущая архитектура маркетплейса — в [`marketplace-notes.md`](marketplace-notes.md).
