...
который пишет в группу ботом самого исполнителя; сырые авто-уведомления отключены
...
адресацию по всем семи ролям, негативные кейсы, слэш-команды и **гонку семи процессов за
арбитраж** (ровно один вызов модели, ровно один отвечающий, корректный фолбэк при сбое).
...
1. **`office-gate` v3 не проверен вживую.** v2 провалилась на первом же прогоне (отвечали
   все семеро, дубли карточек) и переписана на арбитра. Логика покрыта тестами, включая
   гонку семи процессов, но на живом чате v3 ещё не гоняли. Известные слабые места:
   ...
   синхронные; `office_report` — инструкция модели, а не техническая гарантия, и забытый
   вызов оставит результат только на доске.
