...
**✅ Работает, проверено живьём 2026-08-01** (группа без бота-участника →
`@edith_guest_bot представься` → ответ EDITH пришёл тем же сообщением).
Полный путь до этого — ниже, для истории и на случай будущей отладки.
...
## История первого деплоя (2026-08-01) — три проблемы подряд, все решены

1. **Общий токен с гейтвеем → `409 Conflict`.** Telegram разрешает только
   одно активное подключение `getUpdates` на токен бота, независимо от
   `allowed_updates`. Мост конкурировал с гейтвеем EDITH за один слот —
   архитектурный тупик, не багфиксился. Решение — раздел «Почему
   бот-двойник» выше: отдельный бот, отдельный токен, отдельный слот.
2. **Guest Mode не включался с первого раза** — молчание без единой
   ошибки в логах (ни 409, ни другой). Причина была человеческая (шаг
   BotFather не был доведён до конца), не программная.
3. **Реальная форма `guest_message` разошлась с changelog'ом.** Ожидались
   отдельные поля `guest_bot_caller_user`/`guest_bot_caller_chat` — живой
   апдейт показал, что их нет: отправитель и чат лежат в обычных
   `from`/`chat`, как в стандартном `Message`. `guest_query_id` совпал с
   ожиданием, caller-объекты — нет. Поправлено по факту, не по догадке
   номер два.

`answerGuestQuery` (`InlineQueryResultArticle`) при этом отработал с первой
попытки без правок — единственная часть плана, которая совпала с
предположением сразу.
...
Ответ должен прийти в тот же чат одним сообщением (подтверждено живьём —
см. «История первого деплоя» выше). Если тишина — смотри
`journalctl --user -u hermes-guest-bridge -f`: не пришёл апдейт от
Telegram вообще → проверь шаг 1 (Guest Mode мог не сохраниться); `getUpdates`
вернул `409` → используешь токен основного бота вместо `GUEST_BOT_TOKEN`;
`answerGuestQuery` вернул ошибку → раз формат уже подтверждён рабочим,
скорее всего проблема в `guest_query_id` (истёк / переиспользован) —
проверь свежесть апдейта.
