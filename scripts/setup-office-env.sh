...
#
# Свой топик профиля — куда падает /sethome-домашний канал и куда office_report
# всегда шлёт отчёт по карточке (см. _send_to_office в plugins/office-gate/__init__.py).
#
...
# OFFICE_TOPIC_MAP только чистится (наследие отменённого эксперимента с
# адресацией отчёта заказчику), заново не пишется — код её больше не читает.
KEYS=(
